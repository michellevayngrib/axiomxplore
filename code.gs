...
// ============================================
// BATCH DATA LOADER - Opens spreadsheet once for all data
// ============================================

/**
 * OPTIMIZED: Batch load all initial data in a single spreadsheet open
 * This reduces multiple spreadsheet opens to just one, significantly improving performance
 * @param {Object} options - Which data sets to include
 * @returns {Object} All requested data in a single response
 */
function getBatchInitialData(options) {
  try {
    var enableCaching = getConfig('ENABLE_CACHING', 'TRUE');
    var cache = CacheService.getScriptCache();
    var cacheKey = 'batch_initial_data';
    ...
    // Get company logo map first (needed for products)
    var companyLogos = {};
    var companyInfoSheet = ss.getSheetByName('COMPANY_INFO');
    if (companyInfoSheet) {
      var companyData = companyInfoSheet.getDataRange().getValues();
      var companyHeaders = companyData[0];
      var companyNameIndex = companyHeaders.indexOf('CompanyName');
      var logoURLIndex = companyHeaders.indexOf('LogoURL');

      if (companyNameIndex !== -1 && logoURLIndex !== -1) {
        for (var i = 1; i < companyData.length; i++) {
          var companyName = companyData[i][companyNameIndex];
          var logoURL = companyData[i][logoURLIndex];
          if (companyName && logoURL) {
            companyLogos[companyName] = convertDriveLinkToDirectURL(logoURL);
          }
        }
      }

      // Also build companyInfo array
      result.companyInfo = [];
      for (var i = 1; i < companyData.length; i++) {
        var company = {};
        for (var j = 0; j < companyHeaders.length; j++) {
          var value = companyData[i][j];
          if (companyHeaders[j] === 'LogoURL' && value) {
            company[companyHeaders[j]] = convertDriveLinkToDirectURL(value);
          } else if (companyHeaders[j] === 'VideoURL' && value) {
            company[companyHeaders[j]] = convertDriveVideoToEmbedURL(value);
          } else {
            company[companyHeaders[j]] = value;
          }
        }
        result.companyInfo.push(company);
      }
    }
    ...
    if (productsSheet) {
      ...
      for (var i = 1; i < productsData.length; i++) {
        ...
        for (var j = 0; j < productsHeaders.length; j++) {
          var value = productsData[i][j];
          var header = productsHeaders[j];
    ...
    var detailsSheet = ss.getSheetByName('PRODUCT_DETAILS');
    if (detailsSheet) {
      var detailsData = detailsSheet.getDataRange().getValues();
      var detailsHeaders = detailsData[0];
      result.productDetails = [];

      for (var i = 1; i < detailsData.length; i++) {
        var detail = {};
        for (var j = 0; j < detailsHeaders.length; j++) {
          detail[detailsHeaders[j]] = detailsData[i][j] || '';
        }
        result.productDetails.push(detail);
      }
    }
    ...
    if (contactsSheet) {
      ...
      for (var i = 1; i < contactsData.length; i++) {
        var contact = {};
        for (var j = 0; j < contactsHeaders.length; j++) {
          contact[contactsHeaders[j]] = contactsData[i][j];
        }
    ...
    if (resourcesSheet) {
      ...
      var resourcesHeaders = resourcesData[0];
      result.resources = [];

      for (var i = 1; i < resourcesData.length; i++) {
        var resource = {};
        for (var j = 0; j < resourcesHeaders.length; j++) {
          resource[resourcesHeaders[j]] = resourcesData[i][j];
        }
    ...
    // Load Glossary
    var glossarySheet = ss.getSheetByName('Glossary');
    if (glossarySheet) {
      var glossaryData = glossarySheet.getDataRange().getValues();
      var glossaryHeaders = glossaryData[0];
      result.glossary = [];

      for (var i = 1; i < glossaryData.length; i++) {
        var term = {};
        for (var j = 0; j < glossaryHeaders.length; j++) {
          term[glossaryHeaders[j]] = glossaryData[i][j] || '';
        }
        result.glossary.push(term);
      }
    }

    // Load LEADERSHIP_TEAM (grouped by CompanyID)
    var leadershipSheet = ss.getSheetByName('LEADERSHIP_TEAM');
    if (leadershipSheet) {
      var leadershipData = leadershipSheet.getDataRange().getValues();
      var leadershipHeaders = leadershipData[0];
      result.teams = {};

      for (var i = 1; i < leadershipData.length; i++) {
        var member = {};
        for (var j = 0; j < leadershipHeaders.length; j++) {
          member[leadershipHeaders[j]] = leadershipData[i][j] || '';
        }
        var companyId = member.CompanyID || '';
        if (companyId) {
          if (!result.teams[companyId]) {
            result.teams[companyId] = [];
          }
          result.teams[companyId].push(member);
        }
      }
    }
      ...
      result.products.forEach(function(product) {
        if (product.Industries && Array.isArray(product.Industries)) {
          product.Industries.forEach(function(industry) {
            if (industry && industry !== 'all') {
              var formatted = industry.charAt(0).toUpperCase() + industry.slice(1);
              industriesSet[formatted] = true;
            }
          });
        }
      });
    }
    result.industries = Object.keys(industriesSet).sort();
    ...
    if (enableCaching === 'TRUE' || enableCaching === true) {
      ...
      try {
        ...
        // Cache might be too large, log but don't fail
        Logger.log('WARNING: Could not cache batch data: ' + e.toString());
      }
    }
    ...
    Logger.log('ERROR in getBatchInitialData: ' + error.toString());
